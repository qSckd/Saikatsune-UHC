...
public class GameManager {
    ...
    public void spawnCombatVillager(Player player) {
        ...
        villager.setCustomName(ChatColor.YELLOW + "(Logger) " + player.getName());
            ...
            if(game.getGameStateManager().getCurrentGameState() instanceof IngameState) {
                if(game.getPlayers().size() == 1) {
                    ...
                    for (UUID players : game.getPlayers()) {
                        ...
                        Bukkit.getOnlinePlayers().forEach(player -> player.sendTitle(new Title(CC.translate("&6&l" + allPlayers.getName()),CC.translate("&fhas won the game."),20,70,20)));
                        Bukkit.broadcastMessage(prefix + mColor + "Congratulations to " + allPlayers.getName() + " for winning this game!");
                        Bukkit.broadcastMessage(prefix + mColor + "Thank you for playing UHC in MazeMC Network ❤.");
//                        if (allPlayers.getName().equals("HCFAlerts")) {
//                            Bukkit.broadcastMessage(prefix + mColor + "The fucking goat... " + allPlayers.getName());
//                        }

                        PlayerData.getByName(allPlayers.getName()).setWins(PlayerData.getByName(allPlayers.getName()).getWins() + 1);
            ...
            if(game.getGameStateManager().getCurrentGameState() instanceof IngameState) {
                if(game.getTeamManager().getTeams().size() == 1) {
                    ...
                    Bukkit.getOnlinePlayers().forEach(player -> player.sendTitle(new Title(CC.translate("&6&l" + Arrays.toString(stringArray).
                            replace("[", "").replace("]", "")),CC.translate("&fhas won the game."),20,70,20)));
                    Bukkit.broadcastMessage(prefix + mColor + "Thanks for playing UHC in MazeMC Network ❤.");
    ...
    public void executeFinalHeal() {
        ...
        this.playSound();
        Bukkit.getOnlinePlayers().forEach(player -> player.sendTitle(new Title(CC.translate("&6&lFinal Heal"),CC.translate("&astarted."),20,70,20)));
    ...
    public void endGracePeriod() {
        ...
        this.playSound();
        Bukkit.getOnlinePlayers().forEach(player -> player.sendTitle(new Title(CC.translate("&6&lGrace Period"),CC.translate("&cended."),20,70,20)));
    ...
    public void disqualifyPlayer(OfflinePlayer offlinePlayer) {
        ...
        PlayerData.getByName(offlinePlayer.getName()).setDeaths(PlayerData.getByName(offlinePlayer.getName()).getDeaths() + 1);
